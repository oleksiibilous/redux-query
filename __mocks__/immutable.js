...
const fn = (obj) => {
  if (!obj.hasOwnProperty('merge'))
    obj.merge = jest.fn((objNext) => ({ ...obj, ...objNext }))
  if (!obj.hasOwnProperty('get'))
    obj.get = jest.fn((key, initial) => obj[key] || initial)
  if (!obj.hasOwnProperty('set'))
    obj.set = jest.fn((key, val) => {
      obj[key] = val
...
immutable.is = jest.fn(
  (objPrev = {}, objNext = {}) => objPrev.version === objNext.version
)
